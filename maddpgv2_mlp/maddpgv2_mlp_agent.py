# ==========================================================================================================================================================
# maddpgv2 gnn agent class 
# purpose: creates and updates neural network 
# ==========================================================================================================================================================

import torch as T
from utils.nn import maddpgv2_mlp_actor_model, maddpgv2_mlp_critic_model
...
class maddpgv2_mlp_agent:
    ...
    def __init__(self, mode, scenario_name, training_name, discount_rate, lr_actor, lr_critic, num_agents, actor_dropout_p, critic_dropout_p, state_fc_input_dims, actor_state_fc_output_dims, 
        ...
        self.maddpgv2_mlp_target_actor = maddpgv2_mlp_actor_model(model = "maddpgv2_mlp_actor", model_name = None, mode = mode, scenario_name = scenario_name, training_name = training_name, 
                                                                  learning_rate = self.lr_actor, dropout_p = actor_dropout_p, fc_input_dims = state_fc_input_dims + goal_fc_input_dims, 
                                                                  fc_output_dims = state_fc_output_dims, tanh_actions_dims = action_dims)
    ...
    def update_maddpgv2_mlp_target_models(self, tau = None): 
        ...
        for target_actor_parameters, actor_parameters in zip(self.maddpgv2_mlp_target_actor.parameters(), self.maddpgv2_mlp_actor.parameters()):
        ...
        for target_critic_parameters, critic_parameters in zip(self.maddpgv2_mlp_target_critic.parameters(), self.maddpgv2_mlp_critic.parameters()):
    ...
    def select_action(self, mode, state):
        ...
        self.maddpgv2_mlp_actor.eval()
        
        # turn actor local state observations to tensor in actor device
        actor_input = T.tensor(state, dtype = T.float).to(self.maddpgv2_mlp_actor.device)
    ...
    def save_models(self):
        ...
        T.save(self.maddpgv2_mlp_actor.state_dict(), self.maddpgv2_mlp_actor.checkpoint_path)
        T.save(self.maddpgv2_mlp_target_actor.state_dict(), self.maddpgv2_mlp_target_actor.checkpoint_path)
        T.save(self.maddpgv2_mlp_critic.state_dict(), self.maddpgv2_mlp_critic.checkpoint_path)
        T.save(self.maddpgv2_mlp_target_critic.state_dict(), self.maddpgv2_mlp_target_critic.checkpoint_path)
    ...
    def load_models(self):
        ...
        self.maddpgv2_mlp_actor.load_state_dict(T.load(self.maddpgv2_mlp_actor.checkpoint_path, map_location = T.device('cuda:0' if T.cuda.is_available() else 'cpu')))
        self.maddpgv2_mlp_target_actor.load_state_dict(T.load(self.maddpgv2_mlp_target_actor.checkpoint_path, map_location = T.device('cuda:0' if T.cuda.is_available() else 'cpu')))
        self.maddpgv2_mlp_critic.load_state_dict(T.load(self.maddpgv2_mlp_critic.checkpoint_path, map_location = T.device('cuda:0' if T.cuda.is_available() else 'cpu')))
        self.maddpgv2_mlp_target_critic.load_state_dict(T.load(self.maddpgv2_mlp_target_critic.checkpoint_path, map_location = T.device('cuda:0' if T.cuda.is_available() else 'cpu')))