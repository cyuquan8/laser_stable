...
class maddpgv2_mlp:
    ...
    def apply_gradients_maddpgv2_mlp(self, num_of_agents):
        ...
        for agent_index, agent in enumerate(self.maddpgv2_mlp_agents_list):
            ...
            target_critic_q_value_prime = agent.maddpgv2_mlp_target_critic.forward(critic_state_prime, curr_target_actor_actions_prime_cat, critic_goals).flatten()
            
            # mask terminal target q values with 0
            target_critic_q_value_prime[terminal[:, 0]] = 0.0
            
            # obtain critic q value
            critic_q_value = agent.maddpgv2_mlp_critic.forward(critic_state, past_actor_actions_cat, critic_goals).flatten()
