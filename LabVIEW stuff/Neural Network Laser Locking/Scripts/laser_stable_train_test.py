...
from utils.utils import reward, is_terminating, terminating_condition

# general options
SCENARIO_NAME												= "rb_laser_locking" 
MODEL 														= "maddpgv2_mlp" 
MODE 														= "train"
TRAINING_NAME 												= SCENARIO_NAME + "_" + MODEL + "_500_time_steps"
...
def train_test():
	...
	for eps in range(1, NUMBER_OF_EPISODES + 1): 
		...
		while not is_data_obtained:
			...
			try:
				...
				if len(actor_states) != STATE_DIMENSIONS:
					...
					raise Exception(f"State from episode {eps} time step {ep_time_step} is empty / wrong length ... ")  
		...
		while is_terminal == 0:
			...
			np.savetxt(fname = LABVIEW_LOG_DIRECTORY + "/episode_" + str(eps) + "_time_step_" + str(ep_time_step) + "_action.csv", X = actions, delimiter = ',', fmt = '%1.5f')
